...
def run_network(input, network_shape, network_weights):
    ...
    for network_weight in network_weights:
        print("network_weight", np.shape(network_weight))
        print("network_weight", np.shape(current_input))
...
def train_network(input, output, training_rate, network_shape, network_weights):
    ...
    # print("cur_weight_idx", cur_weight_idx)
    ...
    for delta in deltas:
        ...
        # print("network_weights[cur_weight_idx]", network_weights[cur_weight_idx])
        # print("training_rate * np.dot(delta, input_used.T)", training_rate * np.dot(delta, input_used.T))
        cur_weight_idx -= 1
